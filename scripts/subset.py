...
# template for CSS URL
CSS_URL_TEMPLATE = '''url('font-files/{filename}?v={{{font_v}}}') format('{type}')'''
# template for CSS
CSS_TEMPLATE = '''
/* {comment} */
@font-face {{
...
def getUnicodeMap(font :ttLib.TTFont) -> {int:str} :  # type: ignore # codepoint=>glyphname
  ...
  if hasattr(bestCodeSubTable, 'cmap'):
  ...
  for subset in list(subsets) + [{ 'name':'extra' }]:
    ...
    if subset['name'] in fontinfo['subset']:
      ...
      if cmap is not None:
        ...
        outfilesList = []
        for fontinfoOutfile in fontinfo['outfile']:
          ...
          outfilesList.append(CSS_URL_TEMPLATE.format(
            filename=basename(fontinfoOutfile).format(subset=subset['name']),
            type=type,
            font_v=font_v,
          ))
